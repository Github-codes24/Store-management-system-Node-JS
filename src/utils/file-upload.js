...
export const processUploadedFile = async (file = null, fallbackUrl = null, req = null) => {
  ...
  // 2. Extract fallback URL if passed as string, object, or nested body field
  let rawUrl = fallbackUrl;
  if (!rawUrl && req && req.body) {
    rawUrl =
      req.body.image ||
      req.body.imageUrl ||
      req.body.logo ||
  ...
  if (rawUrl) {
    if (typeof rawUrl === 'object' && rawUrl !== null) {
      rawUrl = rawUrl.url || rawUrl.secure_url || rawUrl.location || rawUrl.value || null;
    }
    if (typeof rawUrl === 'string' && rawUrl.trim().length > 0 && !rawUrl.startsWith('data:')) {
      return rawUrl.trim();
    }
  }
